...
const int    C1R = 2;    // Entrada de la señal A del encoder.(Cable amarillo)
const int    C2R = 3;    // Entrada de la señal B del encoder.(Cable verde)
int cvR = 0;
...
//  motorL.setGains(0.15, 0.09, 0.034); // (Kc,Ti,Td)
  ...
  // Configura el Timer0 para PWM de 10 kHz
  TCCR0A = _BV(COM0A1) | _BV(WGM01) | _BV(WGM00); // Modo Fast PWM
  TCCR0B = _BV(CS01); // Establece el prescaler a 8
  OCR0A = 199; // Establece el TOP para 10 kHz

  // Configura el Timer2 para PWM de 10 kHz
  TCCR2A = _BV(COM2A1) | _BV(WGM21) | _BV(WGM20); // Modo Fast PWM
  TCCR2B = _BV(CS21); // Establece el prescaler a 8
  OCR2A = 199; // Establece el TOP para 10 kHz
